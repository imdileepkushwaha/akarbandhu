...
<head id="Head1" runat="server">
  <title>Akar Bandhu Pvt Ltd — Admin Login</title>
	<!--[if lt IE 11]>
	...
	<meta name="description" content="Akar Bandhu Pvt Ltd Admin Portal" />
	...
	<style>
		...
		.ab-hero::before {
			...
			position: absolute;
			inset: -20% -10%;
			background: repeating-linear-gradient(-18deg, transparent, transparent 48px, rgba(255,255,255,0.03) 48px, rgba(255,255,255,0.03) 49px);
			pointer-events: none;
		...
		.ab-hero-inner { position: relative; z-index: 1; max-width: 36rem; }
		.ab-logo-mark {
			display: inline-flex;
			align-items: center;
			gap: 0.85rem;
		...
		.ab-form-wrap { width: 100%; max-width: 380px; }
		.ab-form-logo {
			display: none;
			align-items: center;
			justify-content: center;
		...
		.ab-form-logo svg { width: 36px; height: 36px; color: var(--ab-orange); }
		.ab-form-logo strong {
			font-family: "Outfit", sans-serif;
			font-size: 1.25rem;
			font-weight: 700;
		...
		.ab-form-sub { margin: 0 0 1.75rem; color: var(--ab-muted); font-size: 0.95rem; }
		...
		@keyframes ab-rise { from { opacity: 0; transform: translateY(18px); } to { opacity: 1; transform: translateY(0); } }
		@keyframes ab-drift { from { transform: translateX(0); } to { transform: translateX(48px); } }
		@keyframes ab-pulse { 0%, 100% { transform: scale(1); opacity: 0.7; } 50% { transform: scale(1.04); opacity: 1; } }
		@media (max-width: 900px) {
			.ab-shell { grid-template-columns: 1fr; }
			.ab-hero { min-height: 36vh; padding: 2rem 1.5rem 1.75rem; }
			.ab-logo-mark svg { width: 40px; height: 40px; }
			.ab-brand { font-size: clamp(1.9rem, 7vw, 2.5rem); }
			.ab-panel { align-items: flex-start; padding-top: 1.75rem; }
			.ab-form-logo { display: flex; }
		}
	</style>
</head>
<body class="ab-login">
  ...
  <div class="ab-shell">
	<section class="ab-hero" aria-label="Brand">
		<div class="ab-hero-inner">
			...
			<p class="ab-tagline">Admin portal — manage associates, bookings, plots, and business operations.</p>
		</div>
	</section>
	...
	<section class="ab-panel">
		<div class="ab-form-wrap">
			...
			<p class="ab-form-label">Admin access</p>
			<h2 class="ab-form-title">Welcome back</h2>
			<p class="ab-form-sub">Sign in to the admin control panel.</p>
