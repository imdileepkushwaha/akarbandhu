...
namespace BusinessLogicTier
{
    public class clsAccount
    {
        ...
        public DataTable getLevelIncomeSummery(clsAccount objaccount)
        {
            string str_query = "select cd.*,ud.associatename from ClosingDetail  cd left join associatedetail ud on cd.associateid=ud.AssociateId where 1=1 ";

            if (objaccount.FromDate != DateTime.MinValue && objaccount.ToDate != DateTime.MinValue)
            {
                str_query += " and Convert(datetime,cd.fromdate,101)  >= '" + objaccount.FromDate + "'   and Convert(datetime,cd.todate,101)   <= dateadd(day,1,'" + objaccount.ToDate + "')";
            }

            if (objaccount.UserId != "")
            {
                str_query += "  and cd.associateid = '" + objaccount.UserId + "' ";
            }
        ...
        public DataTable getLevelIncome(clsAccount objaccount, string Fromfdate, string Todate)
        {
            string str_query = "select cd.*,ud.associatename from ClosingDetail  cd left join associatedetail ud on cd.associateid=ud.AssociateId where 1=1 ";

           if (Fromfdate != string.Empty)
            {
                str_query += " and Cast(cd.Fromdate as date)='" + Fromfdate + "'";
            }
            if (Todate != string.Empty)
            {
                str_query += " and Cast(cd.ToDate as date)='" + Todate + "'";
            }
            if (objaccount.UserId != "")
            {
                str_query += "  and cd.associateid = '" + objaccount.UserId + "' ";
        ...
        public DataTable getRewardReport(clsAccount objaccount, string Fromfdate, string Todate)
        {
            string str_query = "select cd.*,ud.associatename, rl.RewardName from RewardLevelAchievementDetail  cd left join associatedetail ud on cd.AssociateId=ud.AssociateId LEFT JOIN Rewardlevelmaster  rl ON rl.LevelNo=cd.LevelNo where 1=1 ";
        ...
        {
            string str_query = "select cd.*,isnull(cd.paymentstatus,'Due') as paymentstatus2,   ud.username,ud.mobile,bm.BankName,ud.BranchName,ud.AccountHolderName,ud.PanNumber,ud.AccountNo,ud.IFSCCode from ROIDetail  cd WITH (nolock) left join userdetail ud WITH (nolock) on cd.userid=ud.userid LEFT JOIN bankmaster bm WITH (nolock) ON ud.BankName=bm.BankId where 1=1 ";
            if (objaccount.FromDate != DateTime.MinValue && objaccount.ToDate != DateTime.MinValue)
            {
        ...
        public DataTable getLevelIncomeUser(clsAccount objaccount, string Fromfdate, string Todate)
        {
            string str_query = "select  isnull(cd.paymentstatus,'Unpaid') as paymentstatus2,cd.*,cd.CommissionAmount-cd.TDS-cd.AdminDeduction AS finalamount,ud.username,ud.AccountHolderName,ud.AccountNo,ud.IFSCCode,ud.BankName,ud.BranchName,ud.PanNumber from ClosingDetail  cd left join userdetail ud on cd.userid=ud.userid where 1=1 and cd.CommissionAmount>0  AND cd.Userlevel<=(SELECT count(id) FROM UserDetail WITH (nolock) WHERE SponserId='" + objaccount.UserId + "')  ";


            if (Fromfdate != string.Empty)
            {
                str_query += " and Cast(cd.Fromdate as date)='" + FromDate + "'";
            }
            if (Todate != string.Empty)
            {
                str_query += " and Cast(cd.ToDate as date)='" + Todate + "'";
        ...
        {
            string str_query = "SELECT td.*,ud.UserName FROM TransactionDetail td WITH (nolock) LEFT JOIN userdetail ud WITH (nolock) ON td.UserID=ud.UserId where td.transactiontype='DirectIncome'  ";

            if (objaccount.FromDate != DateTime.MinValue && objaccount.ToDate != DateTime.MinValue)
            {
                str_query += "  and td.fromdate  >= '" + objaccount.FromDate + "'   and cd.todate   <= '" + objaccount.ToDate + "' ";
            }
            if (objaccount.UserId != "")
            {
                str_query += "  and td.userid = '" + objaccount.UserId + "' ";
...
";
 ...
 FROM PoolLevelMaster pm WITH (nolock) LEFT JOIN closingdetail cl WITH(nolock) ON pm.levelno=cl.UserLevel AND cl.UserId='" + objaccount.UserId + "' order by  pm.LevelNo ";
        ...
        {
            string str_query = "select cd.*,ud.Associatename,'Matching Income on date '+ convert(nvarchar, cd.closingdate,103) as remark from closingdetail cd with (nolock) left join Associatedetail ud with (nolock) on cd.userid=ud.Associateid where 1=1   ";
            if (objaccount.FromDate != DateTime.MinValue && objaccount.ToDate != DateTime.MinValue)
            {
                str_query += "  and convert(date, cd.mentiondate ) >= convert(date,'" + objaccount.FromDate + "')   and convert(date,cd.mentiondate  ) <= convert(date,'" + objaccount.ToDate + "') ";
            }
        ...
        public DataTable getDirectIncomeNew(clsAccount objaccount)
        {
            string str_query = "select cd.*,ud.Associatename,pd.accountno from directincomeclosingdetail cd with (nolock) left join Associatedetail ud with (nolock) on cd.userid=ud.Associateid left join paymentdetail pd with(nolock) on pd.id=cd.paymentdetailid where 1=1   ";
            if (objaccount.FromDate != DateTime.MinValue && objaccount.ToDate != DateTime.MinValue)
            {
                str_query += "  and convert(date, cd.fromdate ) >= convert(date,'" + objaccount.FromDate + "')   and convert(date,cd.todate  ) <= convert(date,'" + objaccount.ToDate + "') ";
            }
        ...
        public DataTable getVillaFundIncome(clsAccount objaccount)
        {
            string str_query = "select cd.*,ud.Associatename from VillaFundIncomeDetail cd with (nolock) left join Associatedetail ud with (nolock) on cd.userid=ud.Associateid where 1=1   ";
            if (objaccount.FromDate != DateTime.MinValue && objaccount.ToDate != DateTime.MinValue)
            {
                str_query += "  and convert(date, cd.fromdate ) >= convert(date,'" + objaccount.FromDate + "')   and convert(date,cd.todate  ) <= convert(date,'" + objaccount.ToDate + "') ";
            }
        ...
        public string Approve_AssociateWithdrawlRequest(clsAccount objaccount)
        {
            string res = "";
            string s2 = "";
            SqlConnection cn;
            ...
            try
            {
                s2 = "select * from Associatewithdrawlrequest where id=" + objaccount.WithdrawlRequestId + "   and status='Pending' ";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    s2 = "select isnull(max(transactionid),0)+1 from  AssociateTransactionDetail";
                    ...
                    // admincharge = (objaccount.WithdrawlAmount * 15) / 100;
                    finalamount = objaccount.WithdrawlAmount - admincharge;

                    s2 = " insert into AssociateTransactionDetail (transactionid,cramount,dramount,associateid,transactiontype,remark,mentionby,mentiondate)  values (" + strtransactionid + ",0," + finalamount.ToString() + ",'" + objaccount.UserId + "','Withdrawl','Withdrawl For User Id : " + objaccount.UserId + "','" + objaccount.MentionBy + "',getdate())";
                    ObjData.RunInsUpDelQueryTrans(s2, tr);
                    //s2 = " insert into AssociateTransactionDetail (transactionid,cramount,dramount,userid,transactiontype,remark,mentionby,mentiondate)  values (" + strtransactionid + ",0," + objaccount.TDS + ",'" + objaccount.UserId + "','TDS','TDS For User Id : " + objaccount.UserId + "','" + objaccount.MentionBy + "',getdate())";
                    //ObjData.RunInsUpDelQueryTrans(s2, tr);
                    //  s2 = " insert into AssociateTransactionDetail (transactionid,cramount,dramount,userid,transactiontype,remark,mentionby,mentiondate)  values (" + strtransactionid + ",0," + admincharge.ToString() + ",'" + objaccount.UserId + "','Admin & Transaction Charge','Admin & Transaction Charge For User Id : " + objaccount.UserId + "','" + objaccount.MentionBy + "',getdate())";
                    // ObjData.RunInsUpDelQueryTrans(s2, tr);
                    s2 = "update Associatewithdrawlrequest set status='Approved' , approvedate=getdate(),transactionid=" + strtransactionid + ",Paymentmode='" + objaccount.PaymentMode + "', OnlineTransactionId='" + OnlineTransactionId + "' where id=" + objaccount.WithdrawlRequestId + "  ";
                    ObjData.RunInsUpDelQueryTrans(s2, tr);
                    s2 = "update associatedetail set balanceamount=isnull(balanceamount,0)-" + objaccount.WithdrawlAmount + "  where associateid='" + objaccount.UserId + "'  ";
                    ObjData.RunInsUpDelQueryTrans(s2, tr);
        ...
        public string Reject_AssociateWithdrawlRequest(clsAccount objaccount)
        {
            string res = "";
            string s2 = "";
            SqlConnection cn;
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    s2 = "update Associatewithdrawlrequest set status='Rejected' , approvedate=getdate() where id=" + objaccount.WithdrawlRequestId + "  ";
                    ObjData.RunInsUpDelQueryTrans(s2, tr);
        ...
        public DataTable getAssociateTransactionReport(clsAccount objaccount)
        {
            string str_query = "select * from Associatetransactiondetail with (nolock) where   1=1 ";
            ...
            if (objaccount.UserId != "")
            {
                str_query += "  and AssociateId = '" + objaccount.UserId + "' ";
            }
        ...
        public DataTable getAccountBalanceForGetHelpAssociate(clsAccount objaccount)
        {
            string str_query = "SELECT isnull( isnull(Sum(td.cramount),0.00)- isnull(sum(td.dramount),0.00)-( SELECT isnull( sum(wr.Amount),0) FROM  AssociateWithdrawlRequest wr WHERE wr.AssociateId=td.AssociateID AND wr.Status='Pending'),0) FROM AssociateTransactionDetail td   where td.Associateid='" + objaccount.UserId + "' GROUP BY td.AssociateID    ";
        ...
        public string Insert_AssociateWithdrawlRequest(clsAccount objaccount)
        {
            string res = "";
            string s2 = "";
            SqlConnection cn;
            ...
            try
            {
                s2 = "sp_addAssociateWithdrawlRequest";
                SqlParameter[] parameter = {
                new SqlParameter("@AssociateId",objaccount.UserId),
        ...
        {
            string str_query = "select wr.*,convert(DECIMAL(18,2),  (wr.Amount*15)/100) AS admincharge,convert(DECIMAL(18,2), wr.Amount-((wr.Amount*15)/100 ))AS finalamount,ud.associatename,ud.SponserId from withdrawlrequest wr LEFT JOIN associatedetail ud ON wr.UserId=ud.associateid  where 1=1  ";
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                   ...
                   // admincharge = (objaccount.WithdrawlAmount * 15) / 100;
                    ...
                    s2 = " insert into TransactionDetail (transactionid,cramount,dramount,userid,transactiontype,remark,mentionby,mentiondate)  values (" + strtransactionid + ",0," + finalamount.ToString() + ",'" + objaccount.UserId + "','Withdrawl','Withdrawl For User Id : " + objaccount.UserId + "','" + objaccount.MentionBy + "',getdate())";
                    ObjData.RunInsUpDelQueryTrans(s2, tr);
                    ...
                    //ObjData.RunInsUpDelQueryTrans(s2, tr);
                  //  s2 = " insert into TransactionDetail (transactionid,cramount,dramount,userid,transactiontype,remark,mentionby,mentiondate)  values (" + strtransactionid + ",0," + admincharge.ToString() + ",'" + objaccount.UserId + "','Admin & Transaction Charge','Admin & Transaction Charge For User Id : " + objaccount.UserId + "','" + objaccount.MentionBy + "',getdate())";
                   // ObjData.RunInsUpDelQueryTrans(s2, tr);
                    s2 = "update withdrawlrequest set status='Approved' , approvedate=getdate(),transactionid=" + strtransactionid + ",Paymentmode='" + objaccount.PaymentMode + "', OnlineTransactionId='" + OnlineTransactionId + "' where id=" + objaccount.WithdrawlRequestId + "  ";
                    ObjData.RunInsUpDelQueryTrans(s2, tr);
                    s2 = "update associatedetail set balanceamount=isnull(balanceamount,0)-" + objaccount.WithdrawlAmount + "  where associateid='" + objaccount.UserId + "'  ";
                    ObjData.RunInsUpDelQueryTrans(s2, tr);
        ...
        public string Pay_SponserIncome(clsAccount objaccount)
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    s2 = "update SponserClosingDetail set PaymentMode='" + objaccount.PaymentMode + "' ,PaymentStatus='Paid', approvedate=getdate(),OnlineTransactionId='" + objaccount.OnlineTransactionId + "' where  isnull(PaymentStatus,'Unpaid')='Unpaid' AND UserId='" + objaccount.UserId + "'   and convert(date, fromdate ) =convert(date, '" + objaccount.FromDate + "')   and convert(date,todate)  =  convert(date, '" + objaccount.ToDate + "') ";
                    ObjData.RunInsUpDelQueryTrans(s2, tr);
                    res = "t";
                }
                else
        ...
        public string Pay_LevelIncome(clsAccount objaccount)
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    s2 = "update ClosingDetail set PaymentMode='" + objaccount.PaymentMode + "' ,PaymentStatus='Paid', approvedate=getdate(),OnlineTransactionId='" + objaccount.OnlineTransactionId + "' where  isnull(PaymentStatus,'Unpaid')='Unpaid' AND UserId='" + objaccount.UserId + "'   and convert(date, fromdate ) =convert(date, '" + objaccount.FromDate + "')   and convert(date,todate)  =  convert(date, '" + objaccount.ToDate + "') ";
                    ObjData.RunInsUpDelQueryTrans(s2, tr);
                    res = "t";
                }
                else
