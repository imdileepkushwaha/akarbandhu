            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (totalpins >= objFranchise.NoOfEPins)
                    {
                        ...
                        {
                            s2 = " declare @id int ; set @id=(select isnull(max(id),0)+1 from FranchiseEPinTransferHistory) ; insert into FranchiseEPinTransferHistory ( id, EPinNo  ,UserIdFrom  ,    UserIdTo  ,MentionBy ,mentionDate ) values (@id,'" + dtepin.Rows[c]["EPinNo"].ToString() + "','" + objFranchise.FranchiseId + "','" + objFranchise.UserId + "','" + objFranchise.MentionBy + "',getdate() ) ";
...
(@id,'" + dtepin.Rows[c]["EPinNo"].ToString() + "','" + objFranchise.UserId + "',null," + dtepin.Rows[c]["amount"].ToString() + ",'Active','" + objFranchise.FranchiseId + "',getDate())";
                            ObjData.RunInsUpDelQueryTrans(s2, tr);
                        }
                        res = "t";
                    }
        ...
        public string Reject_Request(clsFranchise objFranchise)
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    s2 = "update FranchiseEPinRequest set status='Rejected' , approvedate=getdate() where id=" + objFranchise.RequestId + "  ";
                    ObjData.RunInsUpDelQueryTrans(s2, tr);
